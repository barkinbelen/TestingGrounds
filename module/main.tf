locals {
  ...
  use_glue_job_extra_files_s3_key = var.target_bucket_key_extra_glue_job_files == "" ? "artifact/barkin/extra_py_files.zip" : var.target_bucket_key_extra_glue_job_files
...
data "archive_file" "glue_job_extra_zip" {
  ...
  output_path = "/tmp/zip_files/${random_string.track_glue_job_extra_zip_changes[count.index].result}_${timestamp()}.zip"
  depends_on = [
...
resource "null_resource" "check_if_there_are_changed_files" {
  ...
  provisioner "local-exec" {
    command     = "target_dir=${var.extra_py_files_source_dir} main_branch_name=${var.main_branch_name} bucket=${var.script_bucket} key=${local.use_glue_job_extra_files_s3_key} zip_path=${data.archive_file.glue_job_extra_zip[count.index].output_path} ${path.module}/bin/check_for_updated_files.sh"
    interpreter = ["bash", "-c"]
  }
  depends_on = [
    data.archive_file.glue_job_extra_zip
